...
#--------------------------------------------------------- Envio de reportes sin meta diaria, con monto semanal.
...
for fila in sheet.iter_rows(min_row=3, max_row=31, min_col=1, max_col=9, values_only=True):
    ...
    monto_venta_hoy = fila[1] if fila[1] is not None else 0
    
    monto_venta_semanal = fila[7]
    ...
    if "@" in correo_destinatario:
        try:
            ...
            cuerpo_mensaje = f'Buenas tardes {nombre_empleado}.\n\nTus ventas en el día de hoy suman un total de ${"{:,.2f}".format(monto_venta_hoy)}\n\nEsta semana ${"{:,.2f}".format(monto_venta_semanal)}'
