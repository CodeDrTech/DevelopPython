...
def get_vencimientos():
    """Obtiene los vencimientos de todos los clientes activos."""
    conn = connect_to_database()
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute('''
                ...
                SELECT 
                    c.servicio,
                    cl.nombre,
                    date(p.fecha_pago, '+1 month') as proximo_pago,
                    CASE 
                        WHEN date('now') > date(p.fecha_pago, '+1 month') THEN 'En corte'
                        WHEN date('now', '+3 days') >= date(p.fecha_pago, '+1 month') THEN 'Pendiente'
                        WHEN date('now', '+7 days') >= date(p.fecha_pago, '+1 month') THEN 'Cerca'
                        ELSE 'Al día'
                    END as estado
                FROM cuentas c
                INNER JOIN suscripcion s ON c.id = s.cuenta_id
                INNER JOIN clientes cl ON s.cliente_id = cl.id
                INNER JOIN UltimosPagos up ON cl.id = up.cliente_id
                INNER JOIN pagos p ON up.cliente_id = p.cliente_id 
                    AND up.ultima_fecha = p.fecha_pago
            ...
            print(f"Error consultando vencimientos: {e}")
            return []
        finally:
            conn.close()
    return []